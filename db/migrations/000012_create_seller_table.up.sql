...
CREATE TABLE seller (
    ...
    place_of_work_id INTEGER,
    place_of_work_type TEXT CHECK ( place_of_work_type IN ('hall', 'tray', 'kiosk', NULL) ),
    trading_point_id INTEGER,
    trading_point_type TEXT CHECK ( trading_point_type IN ('department_store', 'store', 'tray', 'kiosk', NULL) ),
    PRIMARY KEY (id),
    UNIQUE (phone_number),
